...
```{r}
set.seed(123)
train <- data_exercises %>% sample_frac(.80)
test  <- anti_join(data_exercises, train, by = 'Session_ID')
# Training LM model
...
test %>% 
  ...
  summarise(difference = sum((`Total_Duration` - `pred`)^2)/n())
```
...
```{r}
# NOTE that this is using the same data as was used for duration. And is showing
...
data_exercises %>% 
  ...
  arrange(Date) -> cost_per_session
cost.fit = lm(`Average Cost Per Session` ~ 
                        avg_dew_point +
                        avg_hourly_wind_speed +
                        avg_pressure_sea +
                        avg_visibility +
