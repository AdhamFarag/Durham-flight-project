...
## Cost model
```{r}
clean_data_processed = clean_data_processed %>% filter(Year %in% c(2019, 2020))

clean_data_processed %>% 
  distinct(Session_ID, .keep_all = T) %>%
  mutate(`Cost Per Hour Per Session`=(Duration*`Fuel Price Per Gallon`*6 + 10) / Duration) %>%
  group_by(Date) %>%
  mutate(`Average Cost Per Hour`= mean(`Cost Per Hour Per Session`)) %>%
  arrange(Date) -> daily_average_cost_per_hour

daily_average_cost_per_hour

#fit = lm(`Average Cost Per Hour` ~ `Mean Temp (°C)`, data=daily_average_cost_per_hour)
#summary(fit)
```
